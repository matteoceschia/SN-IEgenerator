...
namespace FLTranslate {
  ...
  falaise::exit_code do_metadata(const FLSimulate::FLSimulateArgs &, datatools::multi_properties &);
...
namespace FLTranslate {
  ...
  //----------------------------------------------------------------------
  falaise::exit_code do_metadata(const FLSimulate::FLSimulateArgs & flSimParameters,
  ...
  falaise::exit_code do_fltranslate(int argc, char *argv[])
  {
    ...
    std::cout << "translate from event " << startevent << " to " << endevent << std::endl;
    ...
    try {
      ...
      datatools::service_manager services("flTranslationServices", "SuperNEMO Translation Services");
      std::string services_config_file = "fltranslate_services.conf"; // local file
	...
	counter = 0;
	//	std::cout << "tracker hits " << grid_id->size() << " to work on." << std::endl;
	for (int& id : *grid_id) {
	  ...
	  //	  std::cout << "tracker hit " << id << " pushed back." << std::endl;
	}
	
	// work on calorimeter hits translation
	counter = 0;
	//	std::cout << "calo hits " << calo_id->size() << " to work on." << std::endl;
	for (int& id2 : *calo_id) {
	  ...
	  snemo::datamodel::calibrated_calorimeter_hit & the_calibrated_calorimeter_hit = new_handle.grab();
	  ...
	  layer = (unsigned int)calo_column->at(counter); // main calo: 0-19; gveto: 0-15
	  row = (unsigned int)calo_row->at(counter); // main calo: 0-12; xwall 0-15
	  ...
	  //	  std::cout << "calo hit " << id2 << " pushed back." << std::endl;
	}
	...
	//	std::cout << "workitem cleared." << std::endl;
      ...
      std::cerr << "fltranslate : Setup/run of translation threw exception" << std::endl;
